...
def predict_path(save_path):
    tex = texture_score(save_path)
    ...
    if reject:
        return {
            ...
            "slug": None,
            "top7": top7  # ส่ง top-7 แม้จะ reject เพื่อ debug ได้
        }
...
def fabric_detail(fabric_name):
    for info in TEXTILE_INFO.values():
        if info.get("slug") == fabric_name:   # <-- ตรงนี้ต้องแก้ให้ match ด้วย
            try:
